...
# Standard tooltip
tooltip() {
	tooltip_html default --text "$(html_encode_pre "$@")"
}
...
.tooltip{-webkit-animation:fadeIn .2s ease 0s;-webkit-animation-fill-mode:forwards;-webkit-box-shadow:0 0 0 1px rgba(0,0,0,.1),0 5px 9px 0 rgba(0,0,0,.4);background:rgba(255,255,185,.95);color:#000;font:small-caption;opacity:0;padding:2px 3px 3px;position:relative}
@-webkit-keyframes fadeIn {	0% { opacity: 0 } 100% { opacity: .9999 } }
</style>
<div class="tooltip">%text%</div>
EOF)

# Variables: color, glyph, text
TM_tooltip_template_styled=$(cat <<'EOF'
<style>
html,body{background:0;border:0;margin:0;padding:0}
body{padding:1px 10px 14px}
pre,code,tt{font-family:Menlo,monaco,monospace;font-size:inherit;margin:0}
.tooltip{-webkit-animation:fadeIn .2s ease 0s;-webkit-animation-fill-mode:forwards;-webkit-border-radius:2px 0 0 2px;-webkit-box-shadow:0 0 0 1px rgba(0,0,0,.1),0 5px 9px 0 rgba(0,0,0,.4);background:rgba(%color%,.95);color:#fff;font:small-caption;opacity:0;position:relative;text-shadow:0 1px 0 rgba(0,0,0,.2)}
...
.text{padding:2px 3px 3px 4px;margin-left:18px}
@-webkit-keyframes fadeIn {	0% { opacity: 0 } 100% { opacity: .9999 } }
</style>
<div class="tooltip"><div class="glyph">%glyph%</div><div class="text">%text%</div></div>
EOF)

# Variables: color, glyph
TM_tooltip_template_styled_notext=$(cat <<'EOF'
<style>
html,body{background:0;border:0;margin:0;padding:0}
body{padding:1px 10px 14px}
.tooltip{-webkit-animation:fadeIn .2s ease 0s;-webkit-animation-fill-mode:forwards;-webkit-border-radius:5px;-webkit-box-shadow:0 0 0 1px rgba(0,0,0,.1),0 5px 9px 0 rgba(0,0,0,.4);background:rgba(%color%,.95);color:#fff;font:16px/25px monospace,webdings;height:25px;opacity:0;padding:3px;position:relative;text-align:center;text-shadow:0 1px 0 rgba(0,0,0,.2);width:25px}
@-webkit-keyframes fadeIn {	0% { opacity: 0 } 100% { opacity: .9999 } }
</style>
